...
export default function Home() {
  ...
  const _getTokensAfterRemove = async (_removeLPTokens) => {
    try {
      ...
      //because youre destructuring the returned object, youre required to use the actual name of the returned parameter
      const { etherRemoved, CDRemoved } = await getTokensAfterRemove(
        provider,
        removeLPTokenWei,
        _ethBalance,
        cryptoDevTokenReserve
      );
      // console.log( etherRemoved, CDRemoved);
      setRemoveEther(etherRemoved);
      setRemoveCD(CDRemoved);
    } catch (err) {
      console.error(err);
    }
  };
